 ...
 * 
 * in Transaction.java:
 *     inserted synchronizer and MainThread to handle waiting the transactions as well as doing the actual quiescent
 *     checkpointing process. Increment currentTransactions during every transaction so that a checkpoint can be 
 *     done every 10 transactions. 
 *     
 * in RecoveryMgr.java:
 * 	   added a quiescentRecover method where we append the checkpoint record to the log and flush it to the disk.
 * 
 * in BasicBufferMgr.java:
 * 	   added a flushAllBuffers method which flushes the whole pool of buffers
 * 
 * in BufferMgr.java
 *     added a flushAllBuffers helper method which made the connection between BasicBufferMgr and Transaction
 */
...
public class Transaction {
   ...
   private static int currentTransactions = 0;
   private RecoveryMgr    recoveryMgr;
   private ConcurrencyMgr concurMgr;
   private int txnum;
   private BufferList myBuffers = new BufferList();
   private static Boolean quiescentInProgress = false;
   ...
   public Transaction() {
	   txnum = nextTxNumber();
	  if (quiescentInProgress) {
		  try {
				sync.waitForResource();
			} catch (InterruptedException e) {
			}
	  }
	  if (txnum % 10 == 0) {
		   quiescentInProgress = true;
		   Thread t = new Thread(new MainThread());
		   t.start();
	   }
      recoveryMgr = new RecoveryMgr(txnum);
      concurMgr   = new ConcurrencyMgr();
      synchronized (quiescentInProgress) {
    	  currentTransactions++;
      }
   }
   ...
   class MainThread implements Runnable {
	   public void run() {
			  System.out.println("Checking if transactions are finished");
			  try{
			  while (currentTransactions != 0) {
				  Thread.sleep(1000);
			  }
			  SimpleDB.bufferMgr().flushAllBuffers();
			  recoveryMgr.quiescentRecover();
			  }
			  catch (InterruptedException ie) {}
			  try {
				sync.release();
				quiescentInProgress = false;
			} catch (InterruptedException e) {}
	   }
   }
}
