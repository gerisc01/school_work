/**
 * I made changes below to both read and write by adding a value that increases by 1
 * each time a file is written or read. These are then printed off in the RemoteConnectionImpl
 * class under rollback and commit using the methods I created below: getBlocksRead() and getBlocksWritten().
 * 
 * I ran the CreateStudentDB.java file under SimpleDBClient. I got:
 * 
 * Blocks Read: 462
 * Blocks Written: 292
 */
...
public class FileMgr {
   ...
   private int blocksWritten = 0; //continually updates to give the number of blocks written
   private int blocksRead = 0; //continually updates to give the number of blocks read
   ...
   synchronized void read(Block blk, ByteBuffer bb) {
      try {
         ...
         blocksRead = blocksRead + 1; //updates blocksRead
      }
      catch (IOException e) {
   ...
   synchronized void write(Block blk, ByteBuffer bb) {
      try {
         ...
         blocksWritten = blocksWritten + 1; //updates blocksWritten
      }
      catch (IOException e) {
   ...
   //New
   /**
    * Returns an integer indicating how many blocks have been
    * read in a particular database.
    * @return an integer with the number of blocks read
    */
   ...
   /**
    * Returns an integer indicating how many blocks have been
    * written in a particular database.
    * @return an integer with the number of blocks written
    */
