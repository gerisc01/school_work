...
def delete_items(localDict,noteStore,google_auth):
	eDict = get_evernote_list_items(noteStore)
	notebooks = noteStore.listNotebooks()
...
def add_items_to_yaml(noteStore,yamlFile):
	evernote_dev_token = 'S=s317:U=2a12764:E=14524e2f945:C=13dcd31cd47:P=1cd:A=en-devtoken:V=2:H=4ae170cd15a53f88d0b2150c23fcae0b'
	notebooks = noteStore.listNotebooks()
	for i in notebooks:
		if i.name == "gophersbball17's notebook":
		...
		filter.notebookGuid = i.guid
		notes = noteStore.findNotes(evernote_dev_token,filter,0,100)
		yamlList = yamlFile[notebook_name]
		for n in notes.notes:
			if n.title in yamlList:
				index = yamlList.index(n.title)
				yamlList.pop(index)
		if yamlList != []:
			for j in yamlList:
				create_evernote_list_item(noteStore,i.guid,j)
...
add_items_to_yaml(noteStore,localDict)
